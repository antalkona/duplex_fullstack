...
function server_start() {
    whiteList.forEach((userId) => {
        const caption = `#ServerStart \n <b>✅|Сервер "${serverName}" запущен.</b>\nСтатус: <u>работает</u>\nОшибок при старте: <u>0 ошибок</u>\nВремя запуска(МСК): <u>${currentTime}</u>\n----------\n <b>🔧| КОМАНДЫ:</b>\n /login - авторизация в проекте\n /logout - выход\n /status - статус сервера\n /reboot - перезапуск сервера\n\n <b><u>-📋ЛОГИ СЕРВЕРА:📋-</u></b>`;

        // Путь к изображению
        const imagePath = path.join(__dirname, 'public', 'assets', 'images', 'ServerStart.png');

        // Создание потока файла
        const imageStream = fs.createReadStream(imagePath);

        // Отправка изображения с явным указанием типа содержимого
        bot.sendPhoto(userId, imageStream, { caption, parse_mode: 'HTML', contentType: 'image/png' });
    });
}
