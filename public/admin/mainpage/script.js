...
function navigator1() {
    ...
    let link = document.getElementById('n_link').value;
    let priority = document.getElementById('n_prior').value;
    ...
    fetch('/admin/mainpage/navigator', {
        method: 'POST',
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
...
function closeModal3(){
    ...
    if (container1) {
        container1.innerHTML = ''; // Очищаем содержимое div
        container1.innerHTML = `<h1 class="mod_title">Ученикам ред <button type="button" onclick="closeModal3()" class="btn btn-warning">Close</button>
        </h1>
        <div class="objl disobj">
            <p>Priority</p>
            <p>TEXT</p>
...
function navigatorEdit() {
    ...
    fetch('/navigator', {
        ...
        .then(data => {
            ...
            const mainContainer = document.getElementById('container3');

            console.log(datai);
            console.log(datai.length)
            const cont = document.getElementById('container3')
            for (let i = 0; i < datai.length; i++) {
                // Ваш код, который нужно выполнить для каждого элемента массива
                let lesObj = datai[i].id
                let priObj = datai[i].priority
                ...
                div.innerHTML = `
                    ...
                    <input type="text" value="${txtObj}" class="form-control form-control-sm" id="text-${lesObj}" placeholder="Текст">
                    <input type="text" value="${linObj}" class="form-control form-control-sm" id="link-${lesObj}" placeholder="Ссылка">
                    <button type="button" id="edit-${lesObj}" onclick="edit3(${lesObj})" class="btn btn-success">Edit</button>
                    <button type="button" id="del-${lesObj}" onclick="del3(${lesObj})" class="btn btn-danger">Del</button>                   
            `;
                mainContainer.appendChild(div);}
        })
        .catch(error => {
...
function del3(id){
    ...
    console.log(data)
    fetch('/admin/mainpage/navigator/del', {
        method: 'POST',
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
